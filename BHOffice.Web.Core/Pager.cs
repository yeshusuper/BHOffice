...
namespace BHOffice.Web.Core
{
    ...
    {
        public static PageModel<Model> ToPageModel<Model>(this BHOffice.Core.PageModel<Model> model, int start, int limit)
        {
            start = Math.Max(0, start);
            limit = Math.Max(1, limit);

            var pCount = (int)Math.Ceiling((double)model.Count / (double)limit);
            var pIndex = (start / limit) + 1;

            return new PageModel<Model>(model.Models, pIndex, pCount)
            {
